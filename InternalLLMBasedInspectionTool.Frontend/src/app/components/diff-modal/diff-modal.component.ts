import { Component, OnInit, OnDestroy, ElementRef, input, viewChild, effect, signal } from '@angular/core';
...
export class DiffModalComponent implements OnInit, OnDestroy {
  ...
  public diffContainer = viewChild<ElementRef<HTMLDivElement>>('diffContainer');
  ...
  constructor(private readonly sanitizer: DomSanitizer) {
    ...
    effect(() => {
      ...
      if (isOpen && monacoInstance && this.diffContainer()?.nativeElement) {
        if (!this.diffEditor) {
          this.initDiffEditor();
        } else {
  ...
  private async loadMonaco(): Promise<void> {
    try {
      ...
      if (this.isOpen() && this.diffContainer()?.nativeElement && !this.diffEditor) {
        this.initDiffEditor();
      }
    } catch (error) {
